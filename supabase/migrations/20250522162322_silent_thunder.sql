/*
  ...
  1. New Tables
    - `live_sessions`: Stores information about scheduled live sessions
    - `session_attendees`: Tracks attendance for live sessions
  
  2. Enums
    - `session_status`: Status of live sessions (scheduled, live, ended, cancelled)
...
DO $$ BEGIN
  DROP POLICY IF EXISTS "Live sessions are viewable by enrolled students and instructors" ON live_sessions;
  DROP POLICY IF EXISTS "Instructors can manage live sessions" ON live_sessions;
  DROP POLICY IF EXISTS "Users can view session attendance" ON session_attendees;
  DROP POLICY IF EXISTS "Users can join sessions" ON session_attendees;
EXCEPTION
...
DO $$ BEGIN
  ...
  DROP TRIGGER IF EXISTS update_session_attendees_updated_at ON session_attendees;
EXCEPTION
  WHEN undefined_object THEN NULL;
END $$;
